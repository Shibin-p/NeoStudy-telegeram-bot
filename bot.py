...
def log_user(user): logs = load_from_json("logs").get("logs", []) logs.append({ "user_id": user.id, "name": user.full_name, "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S") }) save_to_json({"logs": logs})

async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE): log_user(update.effective_user) intro = ( "📚 Welcome to NeoStudy! 🎓\n" "For KTU CSE (2019 Scheme) students who want quick access to:\n" "📖 Notes | ❓ PYQs | 🎥 Video Classes | 🎧 Voice Notes & more!\n\n" "🛠 Built by Shibin P.\n" "🔻 Tap Start to begin!\n\n" "(You can restart the bot anytime by typing   /start or using Menu button)" ) kb = [[InlineKeyboardButton("🚀 Start", callback_data="sem_menu")], [InlineKeyboardButton("💬 Suggestion", callback_data="suggest")]] await update.message.reply_text(intro, reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
...
async def newsemester(update: Update, ctx: ContextTypes.DEFAULT_TYPE): name = " ".join(ctx.args) data = load_from_json("data") or {} data[name] = {} save_to_json(data) await update.message.reply_text(f"✅ Semester {name} added.", parse_mode="Markdown")

async def newsubject(update: Update, ctx: ContextTypes.DEFAULT_TYPE): name = " ".join(ctx.args) sem = ctx.user_data.get("sem") if not sem: await update.message.reply_text("⚠️ Select semester first.") return data = load_from_json("data") or {} data.setdefault(sem, {})[name] = {t: [] for t in MATERIAL_TYPES} save_to_json(data) await update.message.reply_text(f"✅ Subject {name} added.", parse_mode="Markdown")

async def files(update: Update, ctx: ContextTypes.DEFAULT_TYPE): if ctx.user_data.get("upload"): sem, subj, typ = ctx.user_data["sem"], ctx.user_data["sub"], ctx.user_data["type"] data = load_from_json("data") or {} data.setdefault(sem, {}).setdefault(subj, {}).setdefault(typ, []).append( update.message.document.file_id ) save_to_json(data) await update.message.reply_text("✅ File saved.")
...
async def suggestions(update: Update, ctx: ContextTypes.DEFAULT_TYPE): entry = { "from": update.effective_user.full_name, "id": update.effective_user.id, "text": update.message.text, "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S") } sug = load_from_json("suggestions") or {} sug.setdefault("suggestions", []).append(entry) save_to_json(sug) await update.message.reply_text("✅ Thank you for your suggestion!") await ctx.bot.send_message(chat_id=ADMIN_ID, text=f"📩 Suggestion from {entry['from']}:\n{entry['text']}")

FastAPI server

app_web = FastAPI()

@app_web.api_route("/ping", methods=["GET", "HEAD"]) async def ping(): return {"status": "✅ NeoStudy Bot is alive!"}

def start_fastapi(): import uvicorn uvicorn.run(app_web, host="0.0.0.0", port=10000)

Main

app = ApplicationBuilder().token(TOKEN).build() app.post_init = set_menu_button app.add_handler(CommandHandler("start", start)) app.add_handler(CommandHandler("newsemester", newsemester)) app.add_handler(CommandHandler("newsubject", newsubject)) app.add_handler(CommandHandler("finish", finish)) app.add_handler(CallbackQueryHandler(cb)) app.add_handler(MessageHandler(filters.Document.ALL, files)) app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, suggestions)) app.add_handler(CommandHandler("suggest", suggest_command))

print("🤖 Bot running …")

if name == "main": Thread(target=start_fastapi).start() app.run_polling()
