...
ADMIN_ID = 1457980555  # Replace with your Telegram user ID
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    intro = (
        ...
        "🛠 Built by Shibin P.\n"
        "🔻 Tap Start to begin!\n\n"
        "(You can restart the bot anytime by typing /start or using Menu button)"
    )
    kb = [
        [InlineKeyboardButton("🚀 Start", callback_data="sem_menu")],
        [InlineKeyboardButton("💬 Suggestion", callback_data="suggest")]
    ]
    if update.effective_user.id == ADMIN_ID:
        kb.append([InlineKeyboardButton("📦 Backup", callback_data="backup")])
...
async def files(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if ctx.user_data.get("upload"):
        ...
        data = load_from_json("data") or {}
        data.setdefault(sem, {}).setdefault(subj, {}).setdefault(typ, []).append(update.message.document.file_id)
        save_to_json("data", data)
